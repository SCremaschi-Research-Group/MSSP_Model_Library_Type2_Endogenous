...
def MSSP_model(T, I, S, cmng_t, cproc_t, c1_t, M_t, P_t, a0_is, a1_is, g_is, D_ssp, IJ, probability, CMadd):
	...
	### Objective function ###
	def D1(model):
		return sum(probability[s]*sum(((c1_t[t]*model.a1_is[i,s] - cproc_t[t]*model.a0_is[i,s])*model.z_its[i,t,s] - model.a0_is[i,s]*cmng_t[t]*model.y_its[i,t,s]) for i in I for t in T) for s in S)
	model.objective = Objective(sense=maximize, rule=D1)
	...
	def D4_23(model,t,s):
		return sum(model.a0_is[i,s]*model.y_its[i,t,s] for i in I) <= M_t[t]
	model.D4_23 = Constraint(T,S, rule = D4_23)
