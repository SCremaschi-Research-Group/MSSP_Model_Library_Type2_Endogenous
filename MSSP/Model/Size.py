...
def MSSP_model(I, T, T_end, S, Cpr_is, D_its, probability, D_ssp, Phi_tssp, c_t, rho, sigma, M, Cpu):
	...
	def A1(model):
		return sum(probability[s]*sum((sum(sigma*model.z_its[i,t,s] + model.Cpr_is[i,s]*model.y_its[i,t,s] for i in I) + rho*sum(model.x_ijts[i,j,t,s] for i in I for j in I if j<i)) for t in T) for s in S)
	model.objective = Objective(sense=minimize, rule=A1)
	# model.objective.pprint()
	
	def A2_33(model,i,j,t,s):
		return sum(model.x_ijts[i,j,t,s] for i in I if i>=j) >= model.D_its[i,t,s]
	model.A2_33 = Constraint(I,I,T,S, rule = A2_33)
	# model.A2_33.pprint()
