...
def MSSP_model(K, K_end, J, S, A, C, f_jjp, R, Q, d_js, D_ssp, k_ssp, probability, Cp):
	...
	### Objective function ###
	def H15(model):
		return sum(probability[s]*(sum(f_jjp[j,jp]*model.delta_jjpks[j,jp,k,s] for j in J for jp in J for k in K if (j,jp,k) in A) + sum(Cp*model.P_ks[k,s] for k in K)) for s in S)
	model.objective = Objective(sense=minimize, rule=H15)
	...
	def H16(model,s):
		return model.y_ks[1,s] == Q - sum(model.d_js[j,s]*model.delta_jjpks[0,j,1,s] for j in J if (0,j,1) in A) + model.P_ks[1,s]
	model.H16 = Constraint(S, rule = H16)
	# model.H16.pprint()
	
	def H17(model,k,s):
		if k>1:
			return model.y_ks[k,s] <= model.y_ks[k-1,s] - sum(model.d_js[jp,s]*model.delta_jjpks[j,jp,k,s] for jp in J for j in J if (j,jp,k) in A) + model.P_ks[k,s]
		else:
			return Constraint.Skip
	model.H17 = Constraint(K,S, rule = H17)
	# model.H17.pprint()
