...
def MSSP_model(I, T, T_end, S,
	...
	# For the linearization of y_itps*Qg_(p-1)s, y_itps*Qo_(p-1)s, and y_itps*Qng_(p-1)s.
	...
	model.LFRALM_irs = Var(I,T,S) # Liquid flow rate at month r
	
	# Used in complete-recourse-added model
	...
	def K1_76(model):
		return sum(probability[s]*(sum((model.GI_rs[r,s] - model.XTI_rs[r,s]*FT)*1/(1+MARR)**r for r in T) - model.CC_s[s]*(1 - FT)) for s in S)
	...
	def K18K19_70(model,r,s):
		if r>=2:
			return model.Qg_rs[r,s] == sum(model.QgALM_irs[i,r,s] for i in I)
		else:
			return model.Qg_rs[1,s] == Qg1
	model.K18K19_70 = Constraint(T,S, rule=K18K19_70)
	# model.K18K19_70.pprint()
	...
	def K25K26_70(model,r,s):
		if r>=2:
			return model.Qo_rs[r,s] == sum(model.QoALM_irs[i,r,s] for i in I)
		else:
			return model.Qo_rs[1,s] == Qo1
	model.K25K26_70 = Constraint(T,S, rule=K25K26_70)
	# model.K25K26_70.pprint()
	...
	def K32K33_70(model,r,s):
		if r>=2:
			return model.Qng_rs[r,s] == sum(model.QngALM_irs[i,r,s] for i in I)
		else:
			return model.Qng_rs[1,s] == Qng1
	model.K32K33_70 = Constraint(T,S, rule=K32K33_70)
	# model.K32K33_70.pprint()
	...
	def K41(model,i,r,s): # LFR = Oil + Natural gas.
		return model.LFRALM_irs[i,r,s] == model.QoALM_irs[i,r,s] + model.QngALM_irs[i,r,s]
	model.K41 = Constraint(I,T,S, rule=K41)
	# model.K41.pprint()	
	
	def K42a(model,i,r,s): # Lower bound for LFR
		return (sum(model.w_ips[i,p,s] for p in T if p<=r) - sum(model.z_its[i,t,s] for t in T if t<=r-1))*LFR_LB_i[i] <= model.LFRALM_irs[i,r,s]
	model.K42a = Constraint(I,T,S, rule = K42a)
	# model.K42a.pprint()

	def K42b(model,i,r,s): # Upper bound for LFR
		return (sum(model.w_ips[i,p,s] for p in T if p<=r) - sum(model.z_its[i,t,s] for t in T if t<=r-1))*LFR_UB_i[i] >= model.LFRALM_irs[i,r,s]
	model.K42b = Constraint(I,T,S, rule = K42b)
	# model.K42b.pprint()
