...
`'diff_set'` specifies the differentiator set associated with this uncertain parameter. In this example, the differentiator set is `D_ssp`, defined as $\mathcal{D}(s,s')=\{i \in \mathcal{I}|C^{pr}_{i,s} \neq C^{pr}_{i,s'}\}$, where $\mathcal{I}$ denotes the set of sizes. <br />
